                    ...
                    <div class="col-sm-12">
                        <div class="page-title-box">
                           <!--  <form class="float-right app-search">
                            ...
                            <h4 class="page-title busines_data"> <i class="fa fa-users"></i>ALL ROLE LIST</h4>
                        </div>
                    </div>
                </div>
...
<div class="col-sm-6">
  @if(session()->get('success_updated'))
   <div class="alert alert-success alert-dismissible fade show">
    <button type="button" class="close" data-dismiss="alert">&times;</button>
      {{ session()->get('success_updated') }}  
    </div>
  @endif
</div>

<div class="col-sm-6">
  @if(session()->get('success_deleted'))
    <div class="alert alert-success alert-dismissible fade show">
    <button type="button" class="close" data-dismiss="alert">&times;</button>
        ...
        <tbody>
            @foreach ($roles as $index => $role)
                <tr id="{{$role->id}}">
                    <td>{{$index+1}}</td>
                    <td>{{$role->role}}</td>
                    <td><a href="/role/{{$role->id}}/edit/" class="btn btn-info"><i class="fa fa-edit"></i></a>
                    ...
                    <!-- <td> -->
                        <a type="button" class="btn btn-danger delete_visitor_btn"  data-toggle="modal"  data-target="#myModal"  id="{{$role->id}}">
...
$(function() {
    $(".delete_visitor_btn").on("click", function() {
        role_id = $(this)[0].id
        delete_user_url = '{{url("/role")}}/'+role_id
        $("#myModal .visitor_delete_btn").attr("id", role_id)
        ...
        $(".visitor_delete_btn").on("click", function(e) {
            ...
            success: function(obj) {
              ...
              if(obj.status ="success") {
                swal(
                    'Success',
                    'The Role deleted <b style="color:green;">successfully</b>!',
                    'success'
                  )
              }
              $("#visitor_table tr#"+role_id).fadeOut()
